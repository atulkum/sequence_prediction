...
class CollField(data.Field):
    def __init__(self, **kwargs):
        super(CollField, self).__init__(**kwargs)

    def preprocess(self, x):
        x_processed = [super(CollField, self).preprocess(s) for s in x]
        return x_processed
...
class DatasetConll2003(object):
    ...
    def __init__(self, config):
        ...
        TEXT_WORD = CollField(pad_token='<pad>', unk_token='<unk>', tokenize=(lambda x: x), sequential=True,
        ...
        train, val, test = SequenceTaggingDataset.splits(
            ...
            separator=' ',
            fields=fields)
        train.examples = [ex for ex in train.examples if ex.word != [u'-DOCSTART-'.lower()]]
        val.examples = [ex for ex in val.examples if ex.word != [u'-DOCSTART-'.lower()]]
        test.examples = [ex for ex in test.examples if ex.word != [u'-DOCSTART-'.lower()]]
